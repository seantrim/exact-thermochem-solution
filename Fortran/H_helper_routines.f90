...
!!internal variables
real*8, parameter :: tol=1.d-7!tol=1.d-10!7.d-12!1.d-14 !!base tolerance for integer tests
real*8 :: m !!elliptic parameter
real*8 :: phi !!Jacobi amplitude
complex*16 :: Fc,Ec,F,E !!elliptic integral values
...
if (abs(cn%RE).gt.1.d0) then
 write(*,*) "JZ error -- abs(cn%RE) is greater than unity"
 write(*,*) "cn=",cn
 stop
elseif (abs(cn%IM).gt.1.d-10) then
 write(*,*) "JZ error -- cn%IM is not zero"
 write(*,*) "u,k,m=",u,k,m
 write(*,*) "cn=",cn
 stop
end if
...
if (RE_periodic.eqv..true.) then
 ...
 if ((fraction_Np_RE.le.abs(tol*Np_RE)).or.((1.d0-fraction_Np_RE).le.abs(tol*Np_RE)).or.(abs(Np_RE).lt.tol)) then
  E%RE=2.d0*Ec%RE*Np_RE-E%RE !!use periodicity of elliptic integrals
 elseif ((fraction_Nm_RE.le.abs(tol*Nm_RE)).or.((1.d0-fraction_Nm_RE).le.abs(tol*Nm_RE)).or.(abs(Nm_RE).lt.tol)) then
  E%RE=2.d0*Ec%RE*Nm_RE+E%RE
 else
  write(*,*) "JZ error -- integer multipliers for real parts do not meet tolerance."
...
if (IM_periodic.eqv..true.) then
 ...
 if ((fraction_Np_IM.le.abs(tol*Np_IM)).or.((1.d0-fraction_Np_IM).le.abs(tol*Np_IM)).or.(abs(Np_IM).lt.tol)) then
  E%IM=2.d0*Ec%IM*Np_IM-E%IM
 elseif ((fraction_Nm_IM.le.abs(tol*Nm_IM)).or.((1.d0-fraction_Nm_IM).le.abs(tol*Nm_IM)).or.(abs(Nm_IM).lt.tol)) then
  E%IM=2.d0*Ec%IM*Nm_IM+E%IM
 else
  write(*,*) "JZ error -- integer multipliers for imaginary parts do not meet tolerance."
